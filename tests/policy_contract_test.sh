...
assert_contains '"ec2:DescribeInstanceCreditSpecifications"' "$bootstrap_script"
assert_contains '"Sid": "ManageEnvironmentParameters"' "$bootstrap_script"
assert_contains '"ssm:PutParameter"' "$bootstrap_script"
assert_contains '--ssm-parameter-path' "$bootstrap_script"
...
assert_contains 'ssm:GetDocument' "$project_root/bin/poorman-aws"
assert_contains 'SSM_SESSION_KMS_KEY_ARN' "$project_root/bin/poorman-aws"
assert_contains 'get-document' "$project_root/bin/poorman-aws"
assert_contains 'api.${values[defaults.target_environment]}.${values[aws.route53_zone_name]}' "$project_root/bin/poorman-aws"
assert_contains "api_url: \${{ vars.API_URL || '__API_URL__' }}" "$project_root/bin/poorman-aws"
assert_contains 'ssm_parameter_path: __SSM_PARAMETER_PATH__' "$project_root/bin/poorman-aws"
assert_contains 'ssm_parameter_path:' "$project_root/.github/workflows/bootstrap-backend-parameters.yml"
assert_contains 'SSM_PARAMETER_PATH:' "$project_root/.github/workflows/bootstrap-backend-parameters.yml"
...
assert_contains 'aws s3 cp "$source_uri/image.tar.zst" "$temporary_path/image.tar.zst"' "$project_root/backend/scripts/activate-release"
